...
`sudo` command is required since `/var/log/messages` is read-only by the `root` user):
    ...
    ```
    $ sudo ./ex5-1.log.py /var/log/messages
    ```
...
what the animal says in the new animals creation method '__init__()'. Neat-O.
...
2. Observe the measurements in TrueSight Intelligence.
